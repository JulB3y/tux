...
void actAltScr(void) { write(STDOUT_FILENO, "\x1b[?1049h\x1b[2J\x1b[H", 14); }

// sents ESC-sequence to stdout to deactivate alt-screen
void deactAltScr(void) { write(STDOUT_FILENO, "\x1b[?1049l", 8); }

// deactivates raw mode
void deactRaw(void) { tcsetattr(STDIN_FILENO, TCSAFLUSH, &orig); }
...
void actRaw(void)
{
        ...
        // disable canonical mode, echo and signals (Ctrl-C etc.)
        raw.c_lflag &= (tcflag_t) ~(ICANON | ECHO | ISIG);

        // disable Ctrl-S / Ctrl-Q flow control. Keep CR handling for Enter key
...
int getGUIApps(char *d, char *n, char *appName, char *execCmd)
{
        ...
        snprintf(path, sizeof(path), "%s/%s", d, n);  // join dir and file
        ...
        while (fgets(line, sizeof(line), f)) {
                ...
                // for each line -> gets key and value and stores name and exec
                // values in vars
        ...
        while (token != NULL) {
                ...
                if (dir != NULL) {
                        while ((ent = readdir(dir)) != NULL) {
                                ...
                                if (getGUIApps(token, ent->d_name, appName,
                                               execCmd)) {
                                        snprintf(desktopPath,
                                                 sizeof(desktopPath), "%s/%s",
                                                 token, ent->d_name);
                                        ...
                                        writeToFile(cacheFile, desktopPath,
                                                    mtime, appName, execCmd);
                                        amount++;
                                }
                        }
                        closedir(dir);
...
// same function as writeAppDataFile, but for macOS
int writeMacAppsFile(const char *cachePath)
{
        const char *appsDirs[] = {
                "/Applications",
                "/System/Applications",
        ...
        for (int d = 0; appsDirs[d]; d++) {
                ...
                while ((ent = readdir(dir))) {
                        ...
                        char execCmd[512];
                        snprintf(execCmd, sizeof(execCmd), "open -a \"%s\"",
                                 name);
                                                        ...
                                                        int *top_n)
{
        ...
        for (int i = 0; i < appAmount; i++) {
                int score = fuzzyScore(queryLower, appList->nameLowerList[i],
                                       queryLen, appList->nameLenList[i]);
...
void onStartUp(int *appAmount, AppList appList)
{
        ...
        snprintf(dataDir, sizeof(dataDir), "%s/.local/share/tux-launcher",
                 homePath);
        mkdir(dataDir, 0755);
        ...
        fcntl(STDIN_FILENO, F_SETFL, flags);
                ...
                const char *appsDir = "/Applications";
        #else
...
void printQuery(char *query, int queryLen)
{
        if (query[0] == 0) {
                ...
                printf("\x1b[%d;%dH\x1b[2K│ %.*s", termRows - 1, 1,
                       termCols - 4, queryLen > termCols - 5 ?
                       query + queryLen - termCols + 4 : query);
        }
        printf("\x1b[%d;%dH│", termRows - 1, termCols);
        printf("\x1b[%d;%dH", termRows - 1, queryLen > termCols - 4 ?
               termCols - 1 : queryLen + 3);
...
void app(void)
{
        ...
        for (;;) {
                ...
                if (ev == 1) {
                        int key = readKey();
                        if (!keyProcessing(key, query, &queryLen, &selected,
                            top, top_n)) {
                                break;
                        }
                        ...
                        if (queryChanged(query, altquery)) {
                                ...
                                selected = 0;
                                oldselc = 0;
                                top = search(top, query, &appList, appAmount,
                                             &top_n);
                                printResults(top, top_n);
                                highlightSelected(selected, top);
                        }
