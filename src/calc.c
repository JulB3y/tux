...
static Token nextToken() {
  ...
  if (isDigitOrDot(input[pos]) || (input[pos] == '-' && isDigitOrDot(input[pos + 1]))) {
    char *endptr;
    double number = strtod(input + pos, &endptr);
    pos += (int)(endptr - (input + pos));
    return (Token){.type = TOKEN_NUMBER, .number = number};
  }

  if (isalpha(input[pos])) {
    int start = pos;
    while (isalpha(input[pos]) && pos - start < 9) {
      pos++;
    }
    int len = pos - start;
    Token token = {.type = TOKEN_FUNCTION};
    strncpy(token.function, input + start, (size_t)len);
    token.function[len] = '\0';
    return token;
  }

  if (isOperator(input[pos])) {
    return (Token){.type = TOKEN_OPERATOR, .op = input[pos++]};
  }
...
static TokenStack *shuntingYard() {
...
int calcHandler(App *app, const char *query) {
  input = query;
  pos = 0;
  ...
  snprintf(app->ui.calc_result, sizeof(app->ui.calc_result), "%.10g", result);
